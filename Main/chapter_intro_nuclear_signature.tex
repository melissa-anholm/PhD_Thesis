...
\input{LeeYangLagrangian.tex}where the coupling constants $C_X^{\pm}$ are written in such a way as to separate out the left-handed ($C_X^{+}$) and right-handed ($C_X^{-}$) components from one another, and the neutrino fields $\nu_{L,R}$ are given a similar treatment.  
...
\note[color=bluetodo]{Seriously, this section needs some citations.  Notably, C.S. Wu~\cite{wu} and Lee+Yang~\cite{LeeYang}.  Perhaps also Hong+Sternberg+Garcia~\cite{hong_sternberg_garcia}.  Probably a bunch more people too though.}
