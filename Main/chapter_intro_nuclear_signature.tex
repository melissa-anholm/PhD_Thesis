...
is worked out in detail within Appendix~\ref{appendix_forthepeople}, this section will simply provide the result directly, and use this as a starting point.  
%As this section is, in part, intended to aid the reader in gaining a physical intuition for decay behaviour, only the leading order terms are described.  This allows 
...
\eea
where $\vecP$ is the overall nuclear spin polarization, and 
$\theta$ is the angle between $\vecP$ and the beta emission direction.  The expression's overall energy dependence has been absorbed into the term $W(\Ebeta)$, so that
\beq
...
%\label{eq:overallenergydependence_intro}
%\eeq
%where we have taken the opportunity to specialize the equations to $\beta^+$ decay transitions with $J = J^\prime = \frac{3}{2}$.  
The remaining parameters can be simplified as:
...
and $E_0$ is the maximum possible $\beta$ energy associated with the transition.
%\aside{Holstein gives a formula for it.  Maybe I should write it out...}   
$\vecJ$ is the  angular momentum vector associated with the parent nucleus, and $J$ its projection onto the axis of quantization. 
...
\eeq
\note[tag]{this section got put verbatim into the appendix.  Have to put something else here.}
...
\eea
where $P_{\mathrm{EC}}$ is the electron capture probability (in the case of $^{37}$K, $P_{\mathrm{EC}}=0.0008$ makes only a small contribution~\cite{SeverijnsTandecki2008}).\aside{could also cite Severijns2008's Refs. 35 and 36...}
