...
We will amend Eq.~\ref{eq:ourdecay} to reflect the presence of $N$ such `shake-off electrons' (SOEs), as
\bea
^{37}\textrm{K} &\rightarrow& \,^{37}\textrm{\!Ar}^{+(N-1)} + \beta^{+} + \nu_e + N \, e_{\textrm{SO}}, 
\label{eq:ourdecay_withsoe}
\eea
where it is clear that, since the parent $^{37}\textrm{K}$ atom was electrically neutral before ($\beta^+$) decay, the daughter $^{37}\textrm{\!Ar}$ will initially have an `extra' orbital electron if none are shaken off.  We also note that the outer electron in an $^{37}\textrm{\!Ar}^{-}$ ion is \emph{not bound},~\aside[color=bluetodo]{cite someone!!  I don't know who.} and in an electric field such as is present within our experimental chamber, this outer electron is removed immediately to be accelerated through the field, leaving behind a neutral $^{37}\textrm{\!Ar}$ atom.

It is useful to consider the energy spectrum of these shake-off electrons.  Guiding our understanding, Levinger~\cite{Levinger} assumes the sudden approximation and simply calculates the overlap in wavefunctions between an electron in the initial atom and an electron in the final state -- either outgoing or bound within the atom.  Analytic expressions can be obtained if the atom can be treated as being hydrogenic.

\note[color=bluetodo]{Where the fuck in Levinger even *are* the equations?  I remember I had this problem before.  Ain't nobody wants to do a contour integral to figure it out.  I think John or Alexandre or someone just fucking wrote them out at some point.  I'd like to just write them down here, otherwise it's weird to talk about some characteristics of the spectrum being more important than others.}
%If the atom is treated as being hydrogenic, analytic expressions can be found.  
