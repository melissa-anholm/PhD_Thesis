...
\mbox{$\bFierz = 0.017 \pm 0.021$} --- consistent with the Standard Model prediction of zero~\cite{Saul2020}. Our measurement is strongly related, yet complementary.  
%
This result for $\bFierz$ cannot be directly compared to our result  in $^{37}$K, because the neutron's sensitivity to scalar and tensor couplings is different than our own.  
%
In terms of non-Standard Model Lorentz current structures, to lowest order in the non-SM  currents the same equation applies:
\bea
\bFierz = \pm \frac{(C_S +C_S') + (C_T - C_T') \lambda^2}{ 1 + \lambda^2}
\eea
where the top/bottom sign is for $\beta^-$/$\beta^+$ decay~\cite{jtw}.~\aside{I think we're missing a $\gamma$.}
...
So our observable is relatively less sensitive to Lorentz tensor currents, and will predominantly constrain or discover Lorentz scalar currents.  See Fig.~(\ref{fig:exclusionplotfromjohn}).
...
The \acf{JTW} beta decay probability density function, including a representation of exotic right-handed couplings, is given, to first order, by Eq.~\ref{equation:jtw_master}.  Here, information about the handedness of any couplings is buried within the relative signs of the primed and un-primed coupling constants ($C_X$ and $C_X^\prime$, for $X = \{ V,A,S,T\}$).  
...
%	\note[tag]{fix caption.}
	\note{reference figure in text somewhere.}
\end{figure}

\begin{figure}[htb]
	\centering
	\includegraphics[width=.999\linewidth]{Figures/Rslow_tof_squished.png}
	\note[tag]{Wait, is this description from John even accurate?  It seems wrong, but my brain won't go.}
	\caption[Recoil TOF Spectra at 535 V/cm]{Recoil TOF Spectra for $^{37}$K, taken at 535 V/cm.  Recoil spectra are partially separated by charge distribution.  $\Rslow$ would manifest as a dip in the centre of the TOF for each charge state.  .... wait, is this even true?!  The large dips visible here are primarily a result of chamber geometry.}
\end{figure}
