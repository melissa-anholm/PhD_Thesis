...
\begin{figure}[h!!!!t!]
	...
	\caption{SOE TOF peaks (eMPC - Scintillator), using the leading edge (LE) and using the trailing edge (TE).}	
	\label{fig:LE_TE}
\end{figure}
...
%	\item Make some more careful cuts to clean the data.  
	%	\begin{itemize}
		...
		\item There was a scintillator timing walk correction before taking the SOE-Beta TOF cut.
		\item Before the scintillator walk correction, there was the thing where the data was re-processed using the leading edge pulse rather than the trailing edge pulse.  Makes it cleaner.  Ben just got stuck with the trailing edge.
		\item Could have implemented an eMCP hit *position* requirement, but that kills too many stats.
	%	\end{itemize}
\end{itemize}
