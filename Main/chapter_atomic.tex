...
\label{section:mot}
\note[note]{Move the content of the other MOT section into here.}
...
One specialized type of magneto-optical trap is the AC-MOT, so named because of its use of a sinusoidal alternating current (AC) within its anti-Helmholtz coils, in contrast to the constant current used in a typical MOT (hereafter referred to as a DC-MOT to eliminate ambiguity).  Although the DC-MOT's robust trapping mechanism cannot be fully matched by a similar AC-MOT, the DC-MOT is very slow to turn off, and it is this latter consideration that led the TRINAT collaboration to implement an AC-MOT for its second trap.
...
Although the trapping mechanism itself can be rapidly destroyed in both an AC- and DC-MOT, the DC-MOT's magnetic field is slow to dissipate and challenging to control during this time.  Because the optical pumping process which (in the present experiment) immediately follows the MOT being shut off requires a uniform magnetic field (in contrast to the uniform magnetic field \emph{gradient} needed for the MOT), we do not consider the MOT to have been fully shut off until any residual magnetic fields have completely dissipated.  
