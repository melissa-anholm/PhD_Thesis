...
%\note[note,nolist]{}
Although recoil-order and other small corrections must be accounted for within a complete description of beta decay, in the case of exotic couplings, leading order \ac{JTW} description is adequate, because any exotic couplings present in nature have already been determined to be either small or nonexistant.  Therefore, in a search for exotic physics of this nature, it is sufficient to describe any exotic coupling parameters with expressions truncated at leading order, in combination with a more precise set of expressions to account for the well-understood physical behaviours that dominate.  
%\note[note,nolist]{}
...
\input{equation_LeeYangHamiltonian.tex}\unskip
...
\note{Blurb continuation:  JTW is for the BSM shit, because they created it before they really knew which form the operators take.~\cite{jtw,jtw_coulomb}  Holstein is for the small contributions to physics that we actually know to exist.~\cite{holstein,holstein_errata}}

\section[JTW Formalism]{JTW Formalism}
\label{sec:jtw_formalism}
Rather than going through the full calculation to find the differential decay rate, I will instead simply give the result, taken from \ac{JTW} and others~\cite{LeeYang,jtw,jtw_coulomb,EbelFeldman1957}.  To leading order, we find a five-dimensional \ac{PDF} for beta decay kinematics:  
%\cite{LeeYang,jtw,jtw_coulomb,EbelFeldman1957}:
%
%
%
...
From an experimental standpoint, within the present experiment it is not possible to reconstruct the neutrino momenta with the available data, because we failed to measure the momenta of the daughters in conjunction with the tagged beta decay events with which we are primarily concerned in this thesis, so there is insufficient kinematic information available.   
...
By choosing the coordinate system describing the outgoing $\beta$-direction to be aligned with the nuclear spin direction ($\vecJ$), it can be seen that any azimuthal dependence has been eliminated with the integral over neutrino direction.  Then, a further integral over the $\phi_\beta$ coordinate yields the result: 
\bea
	\textrm{d}^2 \Gamma  ( \Ebeta, \theta ) 
	&=&
	W(\Ebeta) \left[ 1 + \bFierz \frac{\m c^2}{\Ebeta} + \Abeta \, \frac{v_\beta(\Ebeta) }{c} |\vecP| \cos\theta  \right] \, \dEe \, \textrm{d} \theta , 
\nonumber \\
\label{equation:integrated_jtw_appendix}
\eea
where the polarization $\vecP$ is given by $\vecP := \frac{\vecJ}{J}$, and 
$\theta$ is the angle between the beta emission direction and the polarization direction.
% and is the only angular dependence that remains.  
Here, we have grouped the expression's overall energy dependence into the term $W(\Ebeta)$, so that
\beq
W(\Ebeta) = \frac{2}{(2\pi)^3} \, \FFminus \, \xi \, \pe \Ee (E_0 - \Ee)^2, 
\label{eq:overallenergydependence_appendix}
\eeq
where we have taken the opportunity to specialize the equations to $\beta^+$ decay transitions with $J = J^\prime = \frac{3}{2}$.  The remaining parameters can be simplified as:
\input{equations_jtw_params_onlyintegrated}
\unskip \!\!\!
It is clear that the results of Eqs.~(\ref{equation:integrated_jtw_appendix})-(\ref{eq:jtw_Abetaxi_integrated}) present a significant simplification over the description of Eqs.~(\ref{equation:jtw_master})-(\ref{eq:kronecker}).
...
\input{equation_holstein52}\unskip  % equation:holstein52
...
To properly compare the Holstein and JTW formalisms, Eqs.~(\ref{eq:jtw_holstein_xi})-(\ref{eq:jtw_holstein_Abetaxi}) must be evaluated with $C_S \!=\! C_S^\prime \!=\! C_T \!=\! C_T^\prime = 0$, \mbox{$a_2 \!=\! b \!=\! c_2 \!=\! d \!=\! e \!=\! f \!=\! g \!=\! h \!=\! j_2 \!=\! j_3 \!=\! 0$}, and $M = \infty$.  We find, from Eq.~(\ref{eq:jtw_holstein_xi}):
\bea
...
\label{eq:c1_def}
\eea
We note that in the above expressions, the terms ${G_v \cos\theta_c}$, $(C_V + C_V^\prime )$, and \mbox{$(C_A + C_A^\prime )$} are universally applicable, while the portion of the couplings dependent on the structure of the nucleus resides entirely within the nuclear matrix elements $M_F$ and $M_{GT}$.
%\note{}
...
%
The sign ambiguity can be attributed to a sign ambiguity in the original \ac{JTW} publications.  The result is that the literature can't seem to agree on a single sign convention, or even whether the convention ought to be the same in the cases of $\beta^-$/$\beta^+$ decay.  See, for example, Refs.~\cite{dan_thesis,GCBall2000,Raman1978}.  The important point is that, for whichever convention is in use, the equations result in broadly correct physical behaviour.  In the case of $^{37}$K, we can use the fact that the overall sign of $\Abeta$ \emph{must} be negative within our mathematical expressions, because it is experimentally measured to be negative.  
...
\input{equations_jtw_params_LH.tex}
