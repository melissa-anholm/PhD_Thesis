...
Also in my notes, $g_A$ and $c_1$ have a minus sign in front of them too.  I think it's superfluous.  Recall that $M_{GT}$ is negative.   } in addition to the requirements of Eqs.~(\ref{eq:cVcVprime})-(\ref{eq:cAcAprime}) we will henceforth consider only the left-handed scalar and tensor couplings, so that 
\bea
C_S &=& C_S^\prime 
\label{eq:cScSprime} \\
C_T &=& C_T^\prime.
